...
import {
  CreateAlarmModal,
  EditAlarmModal,
} from "../../components/alarms/AlarmModals";
...
export const AlarmsPage: React.FC = () => {
    ...
    try {
      ...
      if (storedUser) {
        ...
        try {
          ...
          console.warn(
            "⚠️ No se pudo verificar usuario con API, usando datos locales:",
            apiError
          );
      ...
      console.warn(
        "⚠️ Usando usuario por defecto debido a error de autenticación"
      );
      setCurrentUser({
        id: 0,
  ...
  const loadAlarms = async () => {
    try {
      ...
      if (userTreatments.length === 0) {
        ...
        setAlarms([]);
        toast("No tienes tratamientos activos con alarmas");
        return;
      }
      ...
      for (const treatment of userTreatments) {
        try {
          ...
          const treatmentAlarms = await apiService.getTreatmentAlarms(
            treatment.id
          );
          ...
          const formattedAlarms = treatmentAlarms.map((alarm) => ({
            ...
            patientName:
              treatment.patient_name || `Paciente #${treatment.patient_id}`,
          ...
          console.log(
            `✅ ${formattedAlarms.length} alarmas agregadas para tratamiento ${treatment.id}`
          );
        } catch (alarmError) {
          console.warn(
            `⚠️ Error obteniendo alarmas del tratamiento ${treatment.id}:`,
            alarmError
          );
  ...
  const handleCreateAlarm = async (alarmData: any) => {
    try {
      ...
      await apiService.createTreatmentAlarm(
        alarmData.treatmentId,
        newAlarmRequest
      );
  ...
  const handleEditAlarm = async (alarmData: AlarmData) => {
    try {
      ...
      await apiService.updateTreatmentAlarm(
        alarmData.treatment_id,
        alarmData.id,
        updateRequest
      );
        ...
        prev.map((a) =>
          a.id === alarmId
            ? {
                ...
                nextTrigger: calculateNextTrigger(a.time, !a.is_active),
              }
            : a
        )
      );
  ...
  const handleToggleSound = async (alarmId: number) => {
    try {
      ...
      setAlarms((prev) =>
        prev.map((a) =>
          a.id === alarmId ? { ...a, sound_enabled: !a.sound_enabled } : a
        )
      );

      toast.success(
        `Sonido ${!alarm.sound_enabled ? "activado" : "desactivado"}`
      );
    } catch (error: any) {
  ...
  const getNextAlarms = () => {
    ...
    return alarms
      .filter(
        (alarm) =>
          alarm.is_active && alarm.nextTrigger && alarm.nextTrigger > now
      )
      .sort(
        (a, b) =>
          (a.nextTrigger?.getTime() || 0) - (b.nextTrigger?.getTime() || 0)
      )
      .slice(0, 5);
  };
          ...
          <div className="space-y-3">
            ...
            {getNextAlarms().length === 0 && (
              <p className="text-center text-gray-500 py-4">
                No hay alarmas programadas para las próximas 24 horas
              </p>
            )}
          </div>
        </Card>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAlarms.map((alarm) => (
                  <tr key={alarm.id} className="hover:bg-gray-50">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div
                          className={`w-2 h-2 rounded-full ${
                            alarm.visual_enabled
                              ? "bg-green-500"
                              : "bg-gray-300"
                          }`}
                          title={
                            alarm.visual_enabled
                              ? "Notificación visual activada"
                              : "Notificación visual desactivada"
                          }
                        ></div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge
                        variant={alarm.is_active ? "success" : "secondary"}
                      >
          ...
          <div className="space-y-4">
            <p className="text-gray-600">
              ...
              <span className="font-semibold">
                {selectedAlarm?.medicationName}
              </span>{" "}
              a las <span className="font-semibold">{selectedAlarm?.time}</span>{" "}
              para {selectedAlarm?.patientName}?
            </p>
            <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
              <p className="text-sm text-yellow-800">
                Esta acción no se puede deshacer. La alarma se eliminará
                permanentemente.
              </p>
            </div>
